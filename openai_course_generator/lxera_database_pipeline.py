...
class LXERADatabasePipeline:
    ...
    async def _run_langgraph_pipeline(
        ...
        try:
            ...
            if job_id:
                await self._update_job_progress(job_id, {
                    'current_phase': 'Running LangGraph pipeline (Planning → Research → Content)',
            ...
            final_state = get_status(thread_id)
            state_values = final_state.get('values', {})
            
            # Extract results
            plan_id = state_values.get('plan_id')
            research_id = state_values.get('research_id')  
            content_id = state_values.get('content_id')
            
            logger.info(f"✅ LangGraph completed - plan_id: {plan_id}, research_id: {research_id}, content_id: {content_id}")
            
            if not content_id:
                logger.error("❌ LangGraph completed but no content_id found")
                return {
                    'pipeline_success': False,
                    'error': 'No content was generated by LangGraph',
                    'content_id': None
                }
    ...
    async def _run_planning_only_pipeline(
        ...
        try:
            ...
            # **PURE LANGGRAPH PLANNING-ONLY IMPLEMENTATION**
            logger.info("🎯 Using LangGraph Planning Node for outline-only generation")
            ...
            # Run LangGraph planning node directly
            initial_state = {
                "job_id": job_id or str(uuid.uuid4()),
            ...
            planning_result = planning_node(initial_state)
    ...
    def _check_planning_completion(self, planning_result) -> bool:
        ...
        try:
            # Handle dict format from LangGraph node execution
    ...
    async def resume_course_generation(
        ...
        try:
            ...
            # TODO: Implement LangGraph-based resume functionality
            logger.warning("⚠️ Resume functionality not yet implemented for pure LangGraph")
            # For now, redirect to full generation
            return await self.generate_course_for_employee(
                employee_id, company_id, assigned_by_id, job_id, 'full'
            )
