...
const InnovationHub = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-smart-beige via-white to-lxera-red/5">
      <Navigation />
      
      {/* Hero Section */}
      <section className="pt-32 pb-20 px-6">
        <div className="max-w-6xl mx-auto text-center">
          <div className="inline-flex items-center bg-lxera-red/10 px-4 py-2 rounded-full text-lxera-red font-medium text-sm mb-6">
            <Lightbulb className="w-4 h-4 mr-2" />
            Innovation Enablement
          </div>
          <h1 className="text-5xl lg:text-6xl font-bold text-business-black mb-6">
            Innovation
            <span className="text-lxera-red"> Hub</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button size="lg" className="bg-lxera-red text-white rounded-xl px-8 transition-all duration-300 hover:scale-105">
                ...
                <Card key={index} className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 group overflow-hidden">
                  <CardHeader className="pb-4">
                    <div className="flex items-start justify-between mb-4">
                      <div className="w-14 h-14 bg-gradient-to-br from-lxera-red/20 to-lxera-red/20 rounded-2xl flex items-center justify-center">
                        <IconComponent className="w-7 h-7 text-lxera-red" />
                      </div>
                      <Sparkles className="w-5 h-5 text-lxera-red/60" />
                    </div>
                    <CardTitle className="text-xl text-business-black group-hover:text-lxera-red transition-colors">
                      {feature.title}
                    </CardTitle>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 gap-3">
                      {feature.capabilities.map((capability, capIndex) => (
                        <div key={capIndex} className="text-sm text-business-black/70 bg-smart-beige/30 rounded-lg px-3 py-2">
                          {capability}
                        </div>
                      ))}
                    </div>
      ...
      {/* Innovation Process */}
      <section className="py-20 px-6 bg-gradient-to-r from-smart-beige/20 to-lxera-red/10">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-business-black mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {innovationProcess.map((phase, index) => (
              <Card key={index} className="text-center border-0 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 relative">
                <CardHeader>
                  <div className="w-16 h-16 bg-gradient-to-br from-lxera-red to-lxera-red rounded-full flex items-center justify-center mx-auto mb-4">
                ...
                {index < innovationProcess.length - 1 && (
                  <div className="hidden md:block absolute -right-4 top-1/2 transform -translate-y-1/2 z-10">
                    <ArrowRight className="w-8 h-8 text-lxera-red" />
                  </div>
                )}
              </Card>
            ))}
