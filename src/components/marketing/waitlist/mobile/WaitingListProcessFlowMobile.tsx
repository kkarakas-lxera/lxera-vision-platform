  ...
  return (
    <div className="relative flex items-start gap-6">
      ...
      <div className="flex-1">
        <Card 
          className={`group border-0 ${isLight ? 'bg-white/90 text-black' : 'bg-gray-800/60 text-white'} backdrop-blur-md rounded-xl transition-all duration-200 ${
        ...
        >
          <CardHeader className="pb-2 px-4 pt-4">
            ...
            <h3 className={`mt-2 font-semibold text-sm ${step.textColor === 'text-white' ? 'text-white' : 'text-black'}`}>
              {step.title}
            </h3>
          </CardHeader>
          <CardContent className="pt-0 px-4 pb-4">
            <p className={`${step.textColor === 'text-white' ? 'text-gray-300' : 'text-gray-700'} text-xs leading-relaxed`}>
              {step.description}
            </p>
          </CardContent>
        </Card>
