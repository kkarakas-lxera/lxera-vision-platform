...
const TransformationStartsSection = () => {
  return (
    <section className="w-full py-20 px-6 text-center bg-gradient-to-b from-smart-beige to-smart-beige/50 relative overflow-hidden">
      {/* Animated background elements */}
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-10 left-10 w-32 h-32 bg-future-green rounded-full animate-float"></div>
        <div className="absolute top-32 right-20 w-20 h-20 bg-future-green rounded-full animate-float" style={{animationDelay: '1s'}}></div>
        <div className="absolute bottom-20 left-1/3 w-16 h-16 bg-future-green rounded-full animate-float" style={{animationDelay: '2s'}}></div>
      </div>
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        {/* Enhanced bridge content with animations */}
        <div className="bg-white/40 backdrop-blur-sm rounded-2xl p-6 max-w-2xl mx-auto animate-fade-in-scale border border-white/20 hover:bg-white/50 transition-all duration-500" style={{animationDelay: '0.6s'}}>
        ...
        <div className="flex justify-center mt-8 space-x-2 animate-fade-in" style={{animationDelay: '0.8s'}}>
          {[1, 2, 3, 4].map((step, index) => (
            <div 
              key={step}
              className="w-2 h-2 rounded-full bg-future-green/40 animate-pulse"
      ...
      <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1/2">
        <div className="w-16 h-16 bg-white rounded-full shadow-lg flex items-center justify-center border-2 border-future-green/30 animate-bounce-slow hover:scale-110 transition-transform duration-300">
