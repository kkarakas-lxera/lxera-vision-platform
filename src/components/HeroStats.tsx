    ...
    <div className="mt-12">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-5xl mx-auto">
        {stats.map((stat, index) => (
          <div 
            key={index}
            className="flex flex-col items-center p-6 md:p-8 bg-white/85 backdrop-blur-sm rounded-2xl shadow-xl hover:shadow-2xl hover:bg-white/95 hover:scale-105 transition-all duration-300 animate-fade-in-up group relative overflow-hidden focus-within:ring-2 focus-within:ring-brand-accent/50 border border-brand-accent/20"
            style={{
            ...
            tabIndex={0}
          >
            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-br from-brand-accent/8 to-brand-accent/4"></div>
            <stat.icon 
              className="w-8 h-8 mb-4 group-hover:scale-110 transition-transform duration-300 relative z-10 text-brand-accent" 
              aria-hidden="true"
            />
